...
function Invoke-Serve {
    ...
    & $PythonPath -m uvicorn parent_model.api:app --host $ApiHost --port $Port
}
...
function Invoke-CiLocal {
    ...
    & $PythonPath -m pip install -r "parent_model\requirements.txt"
    & $PythonPath -m pip install pandas
    & $PythonPath "models_dataset1\csv_models\export_best_model.py" --metric $Metric --augment
    & $PythonPath -c "import parent_model.api as api; print(api.MODEL_NAME, len(api.FEATURE_COLS))"

    $testSnippet = @'
from fastapi.testclient import TestClient
from parent_model.api import app
...
function Invoke-ServiceAction {
    ...
    $scriptPath = Join-Path $PSScriptRoot "manage_usl_api_service.ps1"
